...
;
      ...
      const FOOD_VISUAL = [
        { re: /arroz/i, e: '🍚', label: 'Arroz', img: 'arroz.jpg' },
        { re: /feij[aã]o/i, e: '🫘', label: 'Feijão', img: 'feijao.jpg' },
        { re: /lentilha/i, e: '🫘', label: 'Lentilha', img: 'lentilha.jpg' },
        { re: /gr[aã]o-?de-?bico|leguminos/i, e: '🫘', label: 'Grão-de-bico', img: 'grao-de-bico.jpg' },
        { re: /frango/i, e: '🍗', label: 'Frango', img: 'frango.jpg' },
        { re: /carne|bovina/i, e: '🥩', label: 'Carne', img: 'carne.jpg' },
        { re: /peixe|sardinha|atum|til[aá]pia/i, e: '🐟', label: 'Peixe', img: 'peixe.jpg' },
        { re: /ovo|omelete/i, e: '🥚', label: 'Ovo', img: 'ovo.jpg' },
        { re: /\btofu\b/i, e: '⬜', label: 'Tofu', img: 'tofu.jpg' },
        { re: /tapioca|cuscuz|beiju/i, e: '⚪', label: 'Tapioca', img: 'tapioca.jpg' },
        { re: /queijo|mussarela|minas/i, e: '🧀', label: 'Queijo', img: 'queijo.jpg' },
        { re: /iogurte/i, e: '🥛', label: 'Iogurte', img: 'iogurte.jpg' },
        { re: /leite|bebida vegetal/i, e: '🥛', label: 'Leite', img: 'leite.jpg' },
        { re: /p[aã]o|torrada|sandu[ií]che/i, e: '🍞', label: 'Pão', img: 'pao.jpg' },
        { re: /aveia|granola/i, e: '🥣', label: 'Aveia', img: 'aveia.jpg' },
        { re: /banana/i, e: '🍌', label: 'Banana', img: 'banana.jpg' },
        { re: /morango/i, e: '🍓', label: 'Morango', img: 'morango.jpg' },
        { re: /ma[cç][aã]/i, e: '🍎', label: 'Maçã', img: 'maca.jpg' },
        { re: /laranja/i, e: '🍊', label: 'Laranja', img: 'laranja.jpg' },
        { re: /abacate/i, e: '🥑', label: 'Abacate', img: 'abacate.jpg' },
        { re: /fruta/i, e: '🍎', label: 'Fruta', img: 'fruta.jpg' },
        { re: /salada|folhas|alface/i, e: '🥗', label: 'Salada', img: 'salada.jpg' },
        { re: /br[oó]colis/i, e: '🥦', label: 'Brócolis', img: 'brocolis.jpg' },
        { re: /cenoura/i, e: '🥕', label: 'Cenoura', img: 'cenoura.jpg' },
        { re: /tomate/i, e: '🍅', label: 'Tomate', img: 'tomate.jpg' },
        { re: /legume|verdura/i, e: '🥗', label: 'Vegetais', img: 'vegetais.jpg' },
        { re: /castanha|amendoim|semente|chia|linha[cç]a/i, e: '🥜', label: 'Oleaginosas', img: 'oleaginosas.jpg' },
        { re: /batata-doce/i, e: '🍠', label: 'Batata-doce', img: 'batata-doce.jpg' },
        { re: /batata|mandioca|aipim/i, e: '🥔', label: 'Batata', img: 'batata.jpg' },
        ...
        { re: /caf[eé]/i, e: '☕', label: 'Café', img: 'cafe.jpg' },
        ...
        { re: /prote[ií]na/i, e: '🥩', label: 'Proteína', img: 'proteina.jpg' }
      ];
      ...
      let geminiKey = localStorage.getItem('health-gemini-key') || '';
      // A IA roda por um servidor próprio (ai-proxy-worker.js). O endereço fica em config.js; visitantes não precisam de chave.
      const AI_PROXY_URL = window.HEALTH_AI_PROXY || '';
      function aiOn() { return !!(geminiKey || AI_PROXY_URL); }
      let chatHistory = [];
      try {
...
${dados}`;
      ...
      async function chamarGemini(pergunta) {
        ...
        const contents = chatHistory.slice(-16).map(h => ({ role: h.role === 'user' ? 'user' : 'model', parts: [{ text: h.text }] }));
        contents.push({ role: 'user', parts: [{ text: pergunta }] });
        const generationConfig = { temperature: 0.8, maxOutputTokens: 2048 };
        const extract = d => ((d && d.candidates && d.candidates[0] && d.candidates[0].content && d.candidates[0].content.parts) || []).map(p => p.text || '').join('');

        if (AI_PROXY_URL) {
          let r = null;
          for (let i = 0; i < 2; i++) {
            try { r = await fetch(AI_PROXY_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ system, contents, generationConfig }) }); } catch (_) { r = null; }
            if (r && (r.ok || r.status === 429 || r.status === 400)) break;
            await new Promise(res => setTimeout(res, 700));
          }
          if (!r) throw new Error('Sem conexão com o assistente. Verifique sua internet.');
          if (r.status === 429) throw new Error('Muitas perguntas em pouco tempo. Aguarde um minuto e tente de novo.');
          if (!r.ok) throw new Error('O assistente está indisponível no momento (' + r.status + ').');
          const d = await r.json(); const t = d.text || extract(d);
          if (!t) throw new Error('A IA não retornou resposta. Tente reformular a pergunta.');
          return t;
        }
        ...
        for (const model of ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-2.5-flash-lite']) {
          for (const withSearch of [true, false]) {
            try {
              ...
              const body = { system_instruction: { parts: [{ text: system }] }, contents, generationConfig };
              if (withSearch) body.tools = [{ google_search: {} }];
              const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
              if (res.status === 401 || res.status === 403) throw new Error('Chave inválida ou sem permissão. Confira a API key no Google AI Studio.');
              if (res.status === 429) throw new Error('Limite da API atingido. Tente de novo em instantes.');
              if (res.status === 400 && withSearch) continue;
              if (res.status === 400) throw new Error('Chave inválida ou pedido recusado. Confira a API key no Google AI Studio.');
              if (res.status === 404) { lastErr = new Error('Modelo indisponível'); break; }
              if (!res.ok) throw new Error('Falha na API (' + res.status + ').');
              const t = extract(await res.json());
              if (!t) throw new Error('A IA não retornou resposta (pode ter sido bloqueada). Tente reformular.');
              return t;
            } catch (e) {
              lastErr = e;
              if (/Chave inválida|Limite da API/.test(e.message)) throw e;
            }
          }
        }
      ...
      async function enviarAi() {
        ...
        try {
          let resposta;
          if (aiOn()) {
            resposta = mdToHtml(await chamarGemini(texto));
